...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !csrf_verify()) {
...
} elseif ($_SERVER['REQUEST_METHOD'] == 'POST' && !check_rate_limit($con, 'forgot_password_writer', $_SERVER['REMOTE_ADDR'] ?? '', 3, 600)) {
    ...
    $error = rate_limit_message($con, 'forgot_password_writer', $_SERVER['REMOTE_ADDR'] ?? '', 600, 'password reset requests');
    ...
    if ($result->num_rows > 0) {
        ...
        try {
            ...
            $resetUrl = rtrim(env('APP_URL'), '/') . '/reset-password?token=' . urlencode($resetToken);
            $emailBody = "
