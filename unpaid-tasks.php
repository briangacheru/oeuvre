<?php include "head.php"; log_activity($con, 'writer', $aid, 'page_view', 'Unpaid'); ?>
...
<?= csrf_field() ?>
                                <?php
                                    $unpaidCountRow = mysqli_fetch_assoc(mysqli_query($con,
                                        "SELECT COUNT(*) AS cnt FROM tbltasks WHERE is_deleted = 0 AND status = 'Completed' AND is_paid = 0 AND email = '$aid'"
                                    ));
                                    $unpaidTaskCount = (int) ($unpaidCountRow['cnt'] ?? 0);
                                ?>
