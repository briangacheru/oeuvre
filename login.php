...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!check_rate_limit($con, 'login_writer', $clientIp, 10, 600)) {
        $loginError = "
            <div class='alert alert-danger alert-dismissible fade show' role='alert'>
                <i class='bi bi-exclamation-circle me-1'></i> " . rate_limit_message($con, 'login_writer', $clientIp, 600, 'login attempts') . "
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        if ($result->num_rows == 1) {
            ...
            $lockedUntil = account_lock_status($con, 'tblwriters', $email);
            ...
            if ($lockedUntil) {
                ...
                $justLocked = register_failed_login($con, 'tblwriters', $email);
